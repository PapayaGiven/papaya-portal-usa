...
const LEVEL_PERKS: Record<CreatorLevel, { title: string; gmvRange: string; perks: string[]; color: string; emoji: string }> = {
  ...
  Rising: {
    ...
    perks: [
      ...
      'Exclusive products unlocked',
      'Priority support',
      'Brand deal alerts (first notification)',
      '€300 GMV badge',
      'Monthly creator briefing',
    ],
  },
  Pro: {
    title: 'Pro',
    ...
    perks: [
      ...
      'Premium campaigns (Pro+ exclusive)',
      'Dedicated creator manager',
      '1:1 strategy calls (monthly)',
      '€1,000 GMV badge',
      'Quarterly bonus',
    ],
  },
  Elite: {
    ...
    perks: [
      'Everything in Pro',
      'All campaigns & highest commissions',
      'Early access to new products',
      'Co-branded deals & agency partnership',
      '€5,000 GMV badge',
    ...
    <div className="min-h-screen bg-brand-light-pink">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8 flex items-center gap-4">
          ...
          />
          <div>
            <h1 className="font-playfair text-4xl text-brand-black mb-1">Your Journey.</h1>
            <p className="font-dm-sans text-gray-500 text-sm">
              Four levels, one mission. See where you stand and what comes next.
            </p>
          </div>
        </div>
        ...
        {creator && (
          <div className="mb-8 inline-flex items-center gap-3 bg-white border border-gray-100 rounded-full px-5 py-2.5 shadow-sm">
            ...
            <span className="font-dm-sans text-sm font-semibold text-brand-black">You are at level</span>
            <span
            ...
            <span className="font-dm-sans text-sm text-gray-400">
              · GMV: €{creator.gmv.toLocaleString('en-DE')}
            </span>
          </div>
        )}
                  ...
                  {isCurrent && creator && LEVEL_CONFIG[level].target && (
                    <div className="mt-5 pt-4 border-t border-gray-50">
                      <div className="flex justify-between mb-1.5">
                        <span className="font-dm-sans text-xs text-gray-400">€{creator.gmv.toLocaleString('en-DE')} GMV</span>
                        <span className="font-dm-sans text-xs text-gray-400">Target: €{(LEVEL_CONFIG[level].target ?? 0).toLocaleString('en-DE')}</span>
                      </div>
